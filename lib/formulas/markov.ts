 ...
 * and the next-state row is averaged over the last draw's values.
 */
export const markov: Formula = (h, K) => {
  const nd = numDigits(K)
  const ds = h.map((draw) => draw.map((v) => digits(v, nd)))
  const probs: number[][] = []
  for (let p = 0; p < nd; p++) {
    const m = Array.from({ length: 10 }, () => new Array<number>(10).fill(1)) // Laplace smoothing
